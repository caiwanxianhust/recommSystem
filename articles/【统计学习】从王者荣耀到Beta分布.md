...
你可能要说，直接计算MVP率，用获得MVP的次数除以总场次，但是如果笔者战绩清零后只打了一场，而且还获得了MVP，那么笔者的MVP率就是100%了，这显然是不合理的，因为根据历史统计数据，我们知道正常业余玩家这个MVP率应该是0.2到0.4之间才对啊，明显会误判了笔者的水平。反之，假如笔者在最开始的几场频频失手没有获取MVP，这可能意味着笔者的MVP率会稍稍低于平均水平，但是不会偏离这个范围太远，直接说笔者是MVP率为0%的菜鸡可不行！
...
在这个Beta分布密度图中，x轴代表的是mvp率。因此请注意在这个实例当中，y轴是概率密度（相当于概率的导数），x轴是mvp率。上图这个Beta分布表示了mvp率的先验知识。
...
$$
Beta (\alpha _0+击中的次数,\beta _0+未击中的次数)
$$
...
我们最感兴趣的结果就是Beta分布的期望，也就是先验知识结合实验数据后的最新估计。复习一下，Beta分布的期望值是$\alpha/(\alpha + \beta)$。因此，在125场比赛获取45次MVP的情况下，新的Beta分布的期望值就是$(30+45)/(30+70+125)=0.333$，比单纯计算概率来估计击球率的方法$45/(125)=0.38$要低，但是比一开始根据玩家平均水平所做的预测30%高了。其实这种方法相当于先将玩家平均水平赋予笔者——也就是让笔者以百场获得30次mvp来作为本赛季的初始数据。
